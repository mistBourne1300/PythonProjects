...
def get_string_float(num, p = False):
	string_num = str(num)
	index_decimal = string_num.find(".")
	leads = string_num[:index_decimal]
	decimals = string_num[index_decimal+1:]
	lead = int(leads)
	lead_string = get_string(lead,p)
	retrun = lead_string
	if len(decimals) > 0 and int(decimals) != 0:
		retrun += "point "
		for d in decimals:
			n = int(d)
			retrun += get_string(n,p)
...
if __name__ == "__main__":
	...
	userNum = float(input(f'Enter a float: '))
	userNumStr = get_string_float(userNum, True)
	print(userNumStr)
